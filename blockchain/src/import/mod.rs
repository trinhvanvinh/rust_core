...
impl<I: BlockImporter> SharedBlockImporter for MutexImporter<I>{
    fn import_block(&self, block:<Self as BlockImporter>::Block)-> Result<(), <Self as BlockImporter>::Error > {
        self.importer.lock().expect("Lock is poisoned").import_block(block)
    }
}
...
impl<I: RawImporter> RawImporter for MutexImporter<I>{
    ...
    fn import_raw(&self, raw: Self::Operation)-> Result<(), Self::Error>{
...
impl<I: RawImporter> SharedRawImporter for MutexImporter<I>{
    fn import_raw(&self, raw: Self::Operation)-> Result<(),<Self as RawImporter>::Error>{
        self.importer.lock().expect("Lock is poisoned").import_raw(raw)
    }
}